...
// The terminator to be used by tests_prompt() to signify the end of its returned
// int array.
...
static clock_t charToIntTests(void)
{
    ...
    {
        rand_strings[i] = random_string(VIS_CHAR_START, VIS_CHAR_END, random_int(MIN_RAND_STR_LEN, MAX_RAND_STR_LEN));
        puts(rand_strings[i]);
    }
    ...
    for (unsigned i = 0; i < DEFAULT_NUM_PASSES; i++)
    {
        ...
        for (; rand_strings[i][j] != '\0'; j++)
        {
            ...
            output[j] = charToInt(rand_strings[i][j]);
...
static int *tests_prompt(void)
{
    ...
    puts(
        "\nEnter the test number(s) you want to run, or \"" RUN_ALL_TESTS_KEYWORD "\" (case sensitive) to run all of the tests.\n"
        "Press the enter/return key to start running the selected tests.");

    /*
     * This array will hold the indexes of the tests the user selects, and will use the value
     * defined by USR_CHOICES_TERMINATOR to terminate the array, hence "(NUM_TESTS + 1)"
     */
    static int choices[NUM_TESTS + 1];

    /*
     * Using a static array allows for this function to be run again without having to incur
     * reallocation.
     * The user should only be entering as many characters as there are digits in NUM_TESTS
     * or characters in RUN_ALL_TESTS_KEYWORD.
     * 
     ...
     */
    static char *input[MAX_INPUT_CHARS < sizeof(RUN_ALL_TESTS_KEYWORD) ? sizeof(RUN_ALL_TESTS_KEYWORD) : (MAX_INPUT_CHARS + 1) ];
    ...
    for (; i < NUM_TESTS; i++)
    {
        ...
        /*
         * If the user enters only a newline, assume the user is finished making choices and exit
         * the input loop
         */
        ...
        /*
         * Overwrites any previous choices to give this keyword a consistent behavior
         */
    ...
    i != NUM_TESTS ? (choices[i] = USR_CHOICES_TERMINATOR) : (choices[NUM_TESTS] = USR_CHOICES_TERMINATOR);
...
int main(void)
{
    ...
    // Omit the amount of time the user took to input their choices from 'global_timer'
